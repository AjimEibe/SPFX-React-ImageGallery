  ...
  public async getImages(siteUrl: string, listId: string, numberImages: number): Promise<any[]> {
    ...
    try {
      ...
      results = await web.lists
        ...
        .top(numberImages)
        .expand('File')
        .filter((`File_x0020_Type eq  'jpg' or File_x0020_Type eq  'png' or  File_x0020_Type eq  'jpeg'  or File_x0020_Type eq  'jfif'  or  File_x0020_Type eq  'gif' or  File_x0020_Type eq  'mp4'`))
        .orderBy('Id')
        .usingCaching()
        .get();
    } catch (error) {
